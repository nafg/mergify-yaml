...
object Action {
  ...
  case class Queue(
      ...
      /** Merge method to use. Possible values are merge, squash, rebase or fast-forward. fast-forward is not supported
        * on queues with speculative_checks > 1, batch_size > 1, or with allow_inplace_checks set to false.
        */
      method: String = "merge",
      /** If method is set to rebase, but the pull request cannot be rebased, the method defined in rebase_fallback will
        * be used instead. Possible values are merge, squash, none. none will report an error if rebase is not possible.
      ...
      /** Method to use to update the pull request with its base branch when the speculative check is done in-place.
        ...
        * against its base branch. Note that the rebase method has some drawbacks, see Using Rebase to Update.
        */
      ...
      /** Premium Plan Feature 🦾 This sets the priority of the pull request in the queue. The pull request with the
        * highest priority is merged first. low, medium, high are aliases for 1000, 2000, 3000. Default: medium
        */
      ...
      /** Whether branch protections are required for queueing pull requests. This option is ignored if the target queue
        * has speculative_checks > 1. Default: true
        */
