...
object Action {
  ...
  case class Merge(
    ...
    /** Mergify can impersonate a GitHub user to merge pull request. If no merge_bot_account is set, Mergify will merge
  ...
  Default: false
      */
    allowMergingConfigurationChange: Option[ToJson /*bool*/] = None,
    /** Deprecated 😵 Whether branch protections are required for queueing pull requests.
  Default: true
      */
  ...
  Default: True
    ...
    autosquash: Option[ToJson /*bool*/] = None,
    /** For certain actions, such as rebasing branches, Mergify has to impersonate a GitHub user. You can specify the
      * account to use with this option. If no bot_account is set, Mergify picks randomly one of the organization users
      * instead. The user account must have already been logged in Mergify dashboard once.
      */
    botAccount: String = ""
  ) extends Action
