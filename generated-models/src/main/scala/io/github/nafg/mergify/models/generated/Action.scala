...
object Action {
  ...
  case class Copy(
      ...
      /** Premium Plan Feature 🦾 Mergify can impersonate a GitHub user to copy a pull request. If no bot_account is
  ...
  case class Merge(
      ...
      /** Premium Plan Feature 🦾 Mergify can impersonate a GitHub user to merge pull request. If no merge_bot_account
  ...
  case class Queue(
      ...
      /** merge for all merge methods except fast-forward where rebase is used | Method to use to update the pull
        * request with its base branch when the | speculative check is done in-place. | Possible values: | | * merge to
        * merge the base branch into the pull request. | * rebase to rebase the pull request against its base branch. |
        * \| Note that the rebase method has some drawbacks, see Using Rebase to Update. | | This option overrides the
        * value defined in the Queue Rules section of the configuration.
        */
      updateMethod: String = ""
  ) extends Action
